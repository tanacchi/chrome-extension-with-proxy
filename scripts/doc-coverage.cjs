...
class DocumentationCoverage {
  ...
  analyzeFile(filePath) {
    try {
      ...
      const functionRegex = /export\s+const\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*(?:\([^)]*\)\s*=>\s*\{|async\s*\([^)]*\)\s*=>\s*\{)/g;
      let match;
      ...
      analysis.coverage = analysis.totalItems > 0 
        ? (analysis.documentedItems / analysis.totalItems) * 100 
        : 100;
   ...
   */
  hasJSDocComment(beforeContent, elementName) {
    // 最後のJSDocコメントを探す
  ...
  run() {
    ...
    // 全体のカバレッジを計算
    this.results.globalCoverage = this.results.totalFiles > 0 
      ? (this.results.documentedFiles / this.results.totalFiles) * 100 
      : 100;
    
    this.results.functionCoverage = this.results.totalFunctions > 0 
      ? (this.results.documentedFunctions / this.results.totalFunctions) * 100 
      : 100;
    
    this.results.classCoverage = this.results.totalClasses > 0 
