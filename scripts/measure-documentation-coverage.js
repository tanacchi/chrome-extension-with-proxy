...
class DocumentationCoverageAnalyzer {
  ...
  calculateCoverage() {
    ...
    return {
      ...
      interfaceCoverage: parseFloat(interfaceCoverage),
      classCoverage: parseFloat(classCoverage),
  ...
  generateReport() {
    ...
    console.log(`   @param付き: ${this.stats.functionsWithParamDocs} (${coverage.paramDocCoverage}%)`);
    ...
    if (this.stats.totalClasses > 0) {
      ...
      console.log(`   ドキュメント化済み: ${this.stats.documentedClasses} (${coverage.classCoverage}%)`);
      console.log('');
    }
